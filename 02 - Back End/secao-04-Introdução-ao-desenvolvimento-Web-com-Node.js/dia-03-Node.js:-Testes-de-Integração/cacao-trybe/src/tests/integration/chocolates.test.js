...
describe('Testando a API Cacao Trybe', function () {
  ...
  describe('Usando o método GET em /chocolates/total', function () {
    it('Retorne a quantidade de chocolates cadastrados', async function () {
  ...
  describe('Usando o método GET em /chocolates/search', function () {
    ...
    it('Pesquise chocolates pelo nome', async function () {
      const response = await chai.request(app)
        .get('/chocolates/search?name=Mo');

      expect(response.status).to.be.equal(200);
      expect(response.body).to.deep.equal(searchResults);
    });

    it('Retorne um array vazio quando não há chocolates que contenham o termo pesquisado', async function () {
      const response = await chai.request(app)
